...
int SIMCOM900::setPinCode(char *pin_code) {
	return SIMCOM900::configandwait(pin_code);
}
...
char GSM::GetNameInPhoneBook(byte position, char *name)
{
	char ret_val = -1;
	...
	SetCommLineStatus(CLS_ATCMD);
	ret_val = 0; // not found yet
	name[0] = 0; // phone number not found yet => empty string
	...
	case RX_FINISHED_STR_RECV:
		// response in case valid name stored:
		// <CR><LF>+CPBR: <index>,<number>,<type>,<text><CR><LF>
		// <CR><LF>OK<CR><LF>
		// ex: +CPBR: 6,"041*******",129,"Finlandia"
		// response in case there is not phone number:
		// <CR><LF>OK<CR><LF>
		// extract phone number string
		// ---------------------------
		...
		if (p_char != NULL) {
			...
			//p_char = ",129,"Finlandia"
			p_char1 = strchr((char *)(p_char1+1), '\"');
			if (p_char1 != NULL) {
				p_char1++;
				p_char = strchr((char *)(p_char1), '\"');
				if (p_char != NULL) {
